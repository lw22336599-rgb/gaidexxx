...
const requestConfig = (config: any): any => {
  ...
  if (localBaseUrl) {
    let localBaseUrlObj = JSON.parse(localBaseUrl)
    config.baseURL = localBaseUrlObj.default
  }
  ...
  if (config.url.includes('fdmvmanager/createclient') || config.url.includes('FdmvManager/CreateClient')) {
...
const handleData = async ({ config, data, status, statusText }: any): Promise<any | any> => {
  ...
  if (code != 205) {
    ...
    gp.$baseMessage(data.message, 'error', 'hey')
    // console.log(code,'code');

    // ElMessage({message:data.message,duration:1000,type:'error',plain:true})
...
/**
 * 更新 axios 实例的默认 baseURL
 * @param baseUrl 新的基础 URL
 */
export const updateRequestBaseUrl = (baseUrl: string) => {
