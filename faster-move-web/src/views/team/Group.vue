<template>
  <div class="page-container">
    <div v-loading="pageLoading" class="page-main">
      <div class="page-item">
        ...
        <div class="item-main">
          <div
            ...
            :key="index"
            class="group-item"
            :class="{ 'is-active-group': index === firstCurrentIndex }"
            @click="selectGroup('first', index)"
          >
            <div class="group-item-name">{{ item.Member.name }}</div>
            <div class="group-item-right">
              <el-popover placement="top-start" trigger="hover">
                <div class="group-tool-list">
                  <div class="group-tool-item" @click="createGroup(item)">
                    <el-icon><plus /></el-icon><span class="group-tool-item-btn">创建分组</span>
                  </div>
                  <div class="group-tool-item" @click="editGroup(item)">
                    <el-icon><edit /></el-icon><span class="group-tool-item-btn">编辑分组</span>
                  </div>
                  <div class="group-tool-item" @click="delGroupForId(item, 'first', index)">
                    <el-icon><delete /></el-icon><span class="group-tool-item-btn">删除分组</span>
                  </div>
                </div>
                <template #reference>
      ...
      <div class="page-item">
        ...
        <div class="item-main">
          <div
            ...
            :key="index"
            class="group-item"
            :class="{ 'is-active-group': index === secondCurrentIndex }"
            @click="selectGroup('second', index)"
          >
            <div class="group-item-name">{{ item.Member.name }}</div>
            <div class="group-item-right">
              <el-popover placement="top-start" trigger="hover">
                <div class="group-tool-list">
                  ...
                  <div class="group-tool-item" @click="delGroupForId(item, 'second', index)">
                    <el-icon><delete /></el-icon><span class="group-tool-item-btn">删除分组</span>
                  </div>
                </div>
                <template #reference>
      ...
      <div class="page-item">
        <el-button
          v-if="secondGroupList && secondGroupList.length > 0"
          :icon="Plus"
          type="primary"
          @click="relevanceShop('second')"
          >关联门店</el-button
        >
        <div class="item-main">
          <div
            v-for="(item, index) in thirdGroupList"
            :key="index"
            class="group-item"
            :class="{ 'is-active-group': index === thirdCurrentIndex }"
            @click="selectGroup('third', index)"
          >
            <div class="group-item-name">{{ item.Member.name }}</div>
            <div class="group-item-right">
              <el-popover placement="top-start" trigger="hover">
                <div class="group-tool-list">
                  ...
                  <div class="group-tool-item" @click="delGroupForId(item, 'third', index)">
                    <el-icon><delete /></el-icon><span class="group-tool-item-btn">删除分组</span>
                  </div>
                </div>
                <template #reference>
      ...
      <div class="page-item">
        <el-button
          v-if="thirdGroupList && thirdGroupList.length > 0"
          :icon="Plus"
          type="primary"
          @click="relevanceShop('third')"
          >关联门店</el-button
        >
      ...
      >
        <div class="drawer-container">
          ...
          <div class="drawer-filter">
            <div class="filter-left">
              <el-segmented
                ...
                size="default"
                @change="setOrganizationValue"
              />
            </div>
            <div class="filter-right">
              <el-select
                v-model="shopListParams.shop_type"
                placeholder="Select"
                style="width: 160px"
                @change="getShopList"
              >
              ...
              <el-cascader
                v-model="shopListParams.shop_city"
                clearable
                :options="cityOptions"
                style="width: 160px"
                @change="getShopList"
          ...
          <div class="filter-table">
              ...
              <el-table-column label="门店信息" prop="date" width="400">
                <template #default="{ row }">
                  <div style="display: flex; align-items: flex-start">
                    ...
                    <div>
                      <div>{{ row.name }}</div>
                      <div style="font-size: 12px; opacity: 0.7">ID: {{ row.office_id }}</div>
                    </div>
                  </div>
                </template>
              </el-table-column>
            ...
            />
            <el-button
              :loading="confirmRelevanceShopState"
              style="margin-top: 10px"
              type="primary"
              @click="confirmRelevanceShop"
              >{{ organizationValue === '未关联分组' ? '关联' : '取消关联' }}门店</el-button
            >
          </div>
        </div>
      </el-drawer>
...
const changeGroup = () => {
  if (
    groupList.value[firstCurrentIndex.value] &&
    groupList.value[firstCurrentIndex.value].children &&
    groupList.value[firstCurrentIndex.value].children.length > 0
  ) {
    secondGroupList.value = groupList.value[firstCurrentIndex.value].children
    if (
      secondGroupList.value[secondCurrentIndex.value].children &&
      secondGroupList.value[secondCurrentIndex.value].children.length > 0
    ) {
...
.page-container {
  ...
  .page-main {
    ...
    width: 100%;
    height: calc(
      var(--vh, 1vh) * 100 - var(--el-nav-height) - var(--el-tabs-height) - var(--el-padding) *
        3 - var(--el-footer-height) - var(--el-padding) * 2 - 2px
    );
    .page-item {
