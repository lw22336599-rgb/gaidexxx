  ...
  <div class="function-price">
    <vab-card>
      <div class="filter-main">
        ...
        <div class="filter-item">
          ...
          <el-input
            v-model.trim="query.priceTitle"
            class="filter-input"
            clearable
            placeholder="输入价格名称关键字"
            size="small"
        ...
        <div class="filter-item">
          ...
          <el-input
            v-model.trim="query.funcCode"
            class="filter-input"
            clearable
            placeholder="输入功能代码精确匹配"
            size="small"
            @keyup.enter="reloadPriceList"
          />
        </div>
        <div class="filter-item filter-item-right">
          ...
          <el-button size="small" @click="reloadPriceList"> 刷新 </el-button>
        </div>
      </div>
    </vab-card>
      ...
      <el-table v-loading="listLoading" :data="priceList" height="calc(100vh - 380px)" style="width: 100%">
        ...
        <el-table-column label="包含功能" min-width="220">
          <template #default="{ row }">
            <div class="func-tags">
              <el-tag
                v-for="name in (row as FunctionPriceWithFuncNameVo).func_names"
                :key="name"
                class="func-tag"
                size="small"
                type="info"
              ...
              <span
                v-if="
                  !(row as FunctionPriceWithFuncNameVo).func_names ||
                  (row as FunctionPriceWithFuncNameVo).func_names.length === 0
                "
              >
                全部功能
              </span>
    ...
    <el-dialog v-model="editDialogVisible" :title="editForm.id ? '编辑功能价格' : '新增功能价格'" width="520px">
      <el-form ref="editFormRef" :model="editForm" :rules="editRules" label-width="90px">
        ...
        <el-form-item label="支持功能">
          ...
          <el-select
            v-model="editForm.multi_func"
            multiple
            filterable
            placeholder="不选择表示支持全部功能"
            size="small"
          >
    ...
    <el-drawer v-model="funcDrawerVisible" title="功能启用管理" size="40%" direction="rtl">
      <el-alert
        ...
        show-icon
        :closable="false"
        title="切换开关将全局禁用/启用该功能，与价格本身无强绑定。"
...
const getPriceList = async () => {
  ...
  try {
    ...
    const rawResult = (await apiManager.functionpriceApi.GetPricePageList(parms)) as any
...
const loadFunctionOptions = async (shopType?: ShopType) => {
  try {
    ...
    const rawResult = (await apiManager.functionpriceApi.GetFuncList(targetShopType)) as any
    // 同样兼容 { code, data: T[] } 与直接返回数组两种情况
    const data: t_wmt_function[] = Array.isArray(rawResult) ? rawResult : rawResult?.data || []
...
const handleDeletePrice = (row: t_wmt_function_price) => {
  ElMessageBox.confirm(`确定要删除价格【${row.title}】吗？`, '提示', {
