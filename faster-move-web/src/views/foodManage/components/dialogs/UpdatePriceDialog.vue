<template>
  <el-dialog v-model="dialogVisible" title="调整原价" width="500px" :close-on-click-modal="false" @close="handleClose">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item :label="form.ChangeType === FoodManageApi.ChangePriceTypeEnum.百分比浮动 ? '百分比' : '价格'" prop="Value">
        <el-input-number v-model="form.Value" :min="-999999" :precision="2" :step="0.1" placeholder="请输入价格"
          style="width: 200px">
          <template #suffix>
        ...
        <span class="ml-2 text-gray-500">
          {{ form.Value > 0 ? '上调' : form.Value < 0 ? '下调' : '' }} {{
            form.ChangeType === FoodManageApi.ChangePriceTypeEnum.百分比浮动 ? '百分比' : '价格' }} </span>
      </el-form-item>
    </el-form>
    <template #footer>
