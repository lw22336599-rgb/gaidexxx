<template>
  <div class="drawerwechat">
    ...
    <div>推送状态：<el-switch v-model="type" class="ml-2" /></div>
    <div class="addwxid">
      <div>推送微信群：</div>
      <div class="addwid">
        ...
        <el-button type="primary" :icon="Plus" text @click="addwx">绑定新微信群</el-button>
      </div>
    </div>
    <div>推送内容设置：</div>
    <div class="tscont">
      <div v-for="item in tipslist" :key="item.name" class="caritem">
        <div class="carttop">
          ...
          <div class="carttips">
            <el-switch v-model="type" class="ml-2" />
            <div style="color: var(--el-color-primary); cursor: pointer; font-size: var(--el-font-size-base)">
              立即推送
            </div>
            <div
    ...
    <div style="color: #aaaaaa">
      (考虑到部分用户，机器人手机不经常带身边所以增加管理员功能，管理员也有权限发送指令绑定门店)
    </div>
  </div>
</template>
