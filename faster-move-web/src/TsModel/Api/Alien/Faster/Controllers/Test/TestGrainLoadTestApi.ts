...
实例创建后会在后台运行（定时器会保持实例激活状态） */
  async CreateInstance(/** Grain 键值，用于标识不同的测试实例 */ grainKey: string): Promise<string> {
...
可以通过多次调用此方法来观察队列堆积情况 */
  async CallTest1(/** Grain 键值 */ grainKey: string): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'POST',
...
用于快速测试，不会造成队列堆积 */
  async CallRetTest2(/** Grain 键值 */ grainKey: string /** 返回值内容 */, retVal: string): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'POST',
...
可以观察当队列过长时，Orleans 是否会拒绝请求 */
  ...
  /** 获取当前并发任务数（用于监控队列情况） */
  async GetActiveTaskCount(/** Grain 键值 */ grainKey: string): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'GET',
...
注意：此方法从外部客户端发送请求，确保请求真正经过 Orleans 消息系统 */
  async SimulateQueueBacklog(
    /** 目标 Grain 键值，向此 Grain 发送请求。如果为空，则使用随机 grainKey */
...
- 观察日志中的 "Dropping expired message" 错误信息 */
  async ReproduceExpiredMessageError(
    /** Grain 键值 */
...
用于监控队列积压情况，帮助诊断消息过期问题 */
  async GetWorkItemGroupQueueInfo(/** Grain 键值 */ grainKey: string): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'GET',
...
检查功能是否正在运行（轻量级检查，仅返回 bool 值） */
  async GetFunctionGrainIsRunning(
    /** 店铺类型 */
