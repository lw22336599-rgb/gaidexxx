...
import type { HttpRequestItemSharPage } from '@/TsModel/Alien/Orleans/Entity/WebView/HttpRequestItemSharPage'
...
export class SharBrowserServiceGrainTestApi {
  ...
  async TestInitialize(/** 主页 URL */ homeUrl: string): Promise<string> {
  ...
  /** 测试 Close - 关闭共享浏览器 */
  async TestClose(/** 主页 URL */ homeUrl: string): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  async TestExecuteScript(/** 主页 URL */ homeUrl: string /** JavaScript 脚本代码 */, script: string): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  async TestHttpRequestOnSharPage(
    /** 主页 URL */
    homeUrl: string /** HTTP 请求参数（包含 SetCookieDomanInfos） */,
    item: HttpRequestItemSharPage
  ...
  /** 测试执行简单脚本 - 获取页面标题 */
  async TestGetPageTitle(/** 主页 URL */ homeUrl: string): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'GET',
  ...
  /** 测试执行简单脚本 - 获取当前 URL */
  async TestGetCurrentUrl(/** 主页 URL */ homeUrl: string): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'GET',
  ...
  async TestGetCookieString(/** 主页 URL */ homeUrl: string): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'GET',
...
4. 关闭浏览器 */
  async TestFullFlow(
    ...
    testRequest?: HttpRequestItemSharPage
  ): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 批量测试多个 HomeUrl 的初始化 */
  async TestBatchInitialize(/** 主页 URL 列表 */ homeUrls: string[]): Promise<Record<string, string>> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  async TestInterceptRequestCompare(
    /** 主页 URL */
