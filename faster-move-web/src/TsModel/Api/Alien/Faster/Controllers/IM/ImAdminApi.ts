...
import type { AdminWorkloadVo } from '@/TsModel/Alien/Faster/Controllers/IM/AdminWorkloadVo'
...
import type { MyWorkloadVo } from '@/TsModel/Alien/Faster/Controllers/IM/MyWorkloadVo'
import type { OnlineAdminsVo } from '@/TsModel/Alien/Faster/Controllers/IM/OnlineAdminsVo'
import type { t_wmt_im_session } from '@/TsModel/Alien/Entity/Tables/IM/t_wmt_im_session'
import type { UpdateImConfigVo } from '@/TsModel/Alien/Faster/Controllers/IM/UpdateImConfigVo'
...
自动从 UserManager 获取当前用户的上级ID，只返回同级在线客服 */
  ...
  /** 获取指定客服的工作负载 */
  async GetAdminWorkload(/** 客服ID（可选，为空则查询当前客服） */ adminId?: string): Promise<AdminWorkloadVo> {
    const config: AxiosRequestConfig = {
      method: 'GET',
...
需要检查上级的 no_auto_kefu 配置，如果上级禁止下级自动开启，则需要报错 */
  ...
  /** 更新当前用户的IM配置 */
  async UpdateImConfig(/** IM配置参数 */ parm: UpdateImConfigVo): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
