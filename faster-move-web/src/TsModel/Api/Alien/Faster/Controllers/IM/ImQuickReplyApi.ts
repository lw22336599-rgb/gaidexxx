...
import type { CreateQuickReplyDto } from '@/TsModel/Alien/Faster/Controllers/IM/CreateQuickReplyDto'
import type { ImportResultVo } from '@/TsModel/Alien/Faster/Controllers/IM/ImportResultVo'
import type { ImportShareCodeDto } from '@/TsModel/Alien/Faster/Controllers/IM/ImportShareCodeDto'
import type { QuickReplyVo } from '@/TsModel/Alien/Faster/Controllers/IM/QuickReplyVo'
import type { ShareCodeInfoVo } from '@/TsModel/Alien/Faster/Controllers/IM/ShareCodeInfoVo'
import type { ShareCodeVo } from '@/TsModel/Alien/Faster/Controllers/IM/ShareCodeVo'
import type { UpdateQuickReplyDto } from '@/TsModel/Alien/Faster/Controllers/IM/UpdateQuickReplyDto'
...
返回当前用户自己创建的话术 + 所有直属下级创建的话术 */
  async GetTeamQuickReplyList(
    /** 页码 */
    page: number /** 每页数量 */,
    pageSize: number
  ): Promise<PageResultVo<QuickReplyVo>> {
    const config: AxiosRequestConfig = {
      method: 'GET',
  ...
  /** 将个人话术转换为团队话术
话术的所有者将变为当前用户的上级 */
  async ConvertToTeamQuickReply(/** 话术ID */ id: string): Promise<QuickReplyVo> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 生成分享码 */
  async CreateShareCode(/** 过期天数（0表示永久有效，默认30天） */ expireDays: number): Promise<ShareCodeVo> {
    const config: AxiosRequestConfig = {
      method: 'POST',
