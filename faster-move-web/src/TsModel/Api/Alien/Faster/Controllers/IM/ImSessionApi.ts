...
import type { ImSessionStatus } from '@/TsModel/Alien/Entity/Enums/IM/ImSessionStatus'
...
支持按状态、店铺筛选，分页查询（默认查询当前登录客服的会话） */
  async GetSessionList(
    ...
    shopId?: string
  ): Promise<PageResultVo<t_wmt_im_session>> {
  ...
  /** 获取会话详情
仅能获取分配给当前客服的会话 */
  async GetSessionDetail(/** 会话ID */ id: string): Promise<t_wmt_im_session> {
    const config: AxiosRequestConfig = {
      method: 'GET',
...
仅能获取分配给当前客服的会话的消息 */
  async GetSessionMessages(
    ...
    page: number /** 每页数量 */,
    pageSize: number
  ): Promise<PageResultVo<t_wmt_im_message>> {
    const config: AxiosRequestConfig = {
      method: 'GET',
...
性能优势：利用 idx_session_sent 索引，无需计算总数，响应更快 */
  async GetSessionMessagesStream(
    /** 会话ID */
    sessionId: string /** 游标时间，获取此时间之前的消息（不包含此时间点）。不传则获取最新50条 */,
    beforeTime?: Date
  ): Promise<t_wmt_im_message[]> {
    const config: AxiosRequestConfig = {
      method: 'GET',
  ...
  /** 回复客户消息
仅能回复分配给当前客服的会话 */
  async ReplyToCustomer(/** 会话ID */ sessionId: string /** 回复请求 */, request: ReplyMessageRequest): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
...
仅能操作分配给当前客服的会话（服务层已验证权限） */
  async AddTag(/** 会话ID */ sessionId: string /** 标签请求 */, request: TagRequest): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 删除会话标签
仅能操作分配给当前客服的会话（服务层已验证权限） */
  async RemoveTag(/** 会话ID */ sessionId: string /** 标签 */, tag: string): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'DELETE',
  ...
  /** 更新会话备注
仅能操作分配给当前客服的会话（服务层已验证权限） */
  async UpdateRemarks(/** 会话ID */ sessionId: string /** 备注请求 */, request: RemarksRequest): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'PUT',
  ...
  /** 关闭会话
仅能操作分配给当前客服的会话（服务层已验证权限） */
  async CloseSession(/** 会话ID */ sessionId: string /** 关闭请求 */, request: CloseSessionRequest): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
