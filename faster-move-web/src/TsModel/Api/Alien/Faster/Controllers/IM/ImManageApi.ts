...
import type { SessionSummaryVo } from '@/TsModel/Alien/Faster/Controllers/IM/SessionSummaryVo'
import type { UnreadCountVo } from '@/TsModel/Alien/Faster/Controllers/IM/UnreadCountVo'
...
export class ImManageApi {
  ...
  /** 标记会话已读
将会话的未读消息数清零 */
  async MarkSessionRead(/** 会话ID */ sessionId: string): Promise<string> {
  ...
  /** 标记消息已读 */
  async MarkMessageRead(/** 消息ID列表 */ messageIds: string[]): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 获取未读消息总数 */
  async GetUnreadCount(/** 店铺ID（可选） */ shopId?: string): Promise<UnreadCountVo> {
    const config: AxiosRequestConfig = {
      method: 'GET',
