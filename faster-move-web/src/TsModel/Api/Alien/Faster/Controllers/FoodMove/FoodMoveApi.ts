...
import type { ActivityType } from '@/TsModel/Alien/Entity/Function/FOODMOVE/Activities/ActivityType'
import type { AddFoodToTaskParms } from '@/TsModel/SiteType/Base/Controllers/AddFoodToTaskParms'
import type { AddGroupToTaskParms } from '@/TsModel/SiteType/Base/Controllers/AddGroupToTaskParms'
import type { FdmvTaskType } from '@/TsModel/Alien/Entity/Function/FOODMOVE/FdmvTaskType'
...
import type { FoodTaskLogError } from '@/TsModel/SiteType/Base/Controllers/FoodTaskLogError'
import type { GetLogParms } from '@/TsModel/SiteType/Base/Controllers/GetLogParms'
import type { ProgressInfo } from '@/TsModel/Alien/Entity/ProgressInfo'
import type { QueryShopItemVo } from '@/TsModel/SiteType/Base/Controllers/QueryShopItemVo'
import type { ShopMoveInfo } from '@/TsModel/SiteType/Base/Models/FoodMove/ShopMoveInfo'
import type { SyncFoodsParms } from '@/TsModel/SiteType/Base/Controllers/SyncFoodsParms'
import type { SyncGroupsParms } from '@/TsModel/SiteType/Base/Controllers/SyncGroupsParms'
import type { t_fdmv_ctgry } from '@/TsModel/Alien/Entity/Tables/function/food_move/t_fdmv_ctgry'
import type { t_wmt_fc_fdmv_log } from '@/TsModel/Alien/Entity/Tables/function/food_move/t_wmt_fc_fdmv_log'
import type { t_wmt_fc_fdmv_task_actcopy } from '@/TsModel/Alien/Entity/Tables/function/food_move/task/t_wmt_fc_fdmv_task_actcopy'
import type { t_wmt_fc_fdmv_task } from '@/TsModel/Alien/Entity/Tables/function/food_move/t_wmt_fc_fdmv_task'
import type { t_wmt_fd_fdmv_task_history } from '@/TsModel/Alien/Entity/Tables/function/food_move/task/t_wmt_fd_fdmv_task_history'
import type { t_wmt_shop_list } from '@/TsModel/Alien/Entity/Tables/shop/t_wmt_shop_list'
import type { UpdateFoodSpuPrice } from '@/TsModel/SiteType/Base/Models/FoodMove/UpdateFoodSpuPrice'
import type { UpdateFoodVoParms } from '@/TsModel/SiteType/Base/Controllers/UpdateFoodVoParms'
...
export class FoodMoveApi {
  ...
  /** 获取新店类目 */
  async GetNewSiteCtgry(
    /** 任务id */
    taskId: string /** 上级id 不填将返回类目 */,
    parentId?: string
  ): Promise<t_fdmv_ctgry[]> {
    const config: AxiosRequestConfig = {
      method: 'GET',
  ...
  async GetGroupFoods(
    ...
    foodName?: string
  ): Promise<PageResultVo<FoodItemVo>> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  async TaskQuery(
    ...
    pageSize: number,
    shopOffId?: string
  ): Promise<PageResultVo<t_wmt_fd_fdmv_task_history>> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 拉取老店活动列表 */
  async PullOldShopActivitys(/** 任务ID */ taskId: string): Promise<ProgressInfo> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 同步活动 */
  async SyncActivitys(taskId: string /** 限定活动类型,哪果不传代表不限制 */, actType?: ActivityType): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 重置新店铺 */
  async ResetNewShop(/** 任务ID */ taskId: string): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 获取商品复制日志 */
  async GetFoodTaskLog(
    taskId: string,
    page: number,
    pageSize: number /** （复制是否成功）不填代表所有日志 */,
    succeed?: boolean
  ): Promise<PageResultVo<FoodTaskLog>> {
    const config: AxiosRequestConfig = {
      method: 'GET',
  ...
  /** 获取任务错误日志 */
  async GetFoodTaskErrLog(
    /** 任务ID */
    taskId: string /** 页码 */,
    page: number /** 每页大小 */,
    pageSize: number /** 商品名称过滤（可选） */,
    foodName?: string
  ): Promise<PageResultVo<FoodTaskLogError>> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 获取活动复制日志 */
  async GetActivityTaskLog(
    taskId: string,
    page: number,
    pageSize: number /** （复制是否成功）不填代表所有 */,
    succeed?: boolean /** 限制活动类型，不传代表所有 */,
    actType?: ActivityType
  ): Promise<PageResultVo<t_wmt_fc_fdmv_task_actcopy>> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 添加分组到任务 */
  async AddGroupToTask(/** 分组对像 */ parms: AddGroupToTaskParms): Promise<string> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  /** 添加分组到任务 */
  async AddGroupToTaskV2(/** 分组对像 */ parms: AddGroupToTaskParms): Promise<FoodGroupItem> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  async ExportTaskFoodData(/** 任务ID */ taskId: string): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
  ...
  async ImportTaskFoodData(
    /** 新店ID（t_wmt_shop_list.id） */
