...
export class FunctionAppApi {
  ...
  async GetShopListFunctionColumns(/** 平台/门店类型 */ shopType: ShopType): Promise<ShopListFunctionColumnVo[]> {
    const config: AxiosRequestConfig = {
      method: 'GET',
  ...
  /** 禁用或启用(上线或下线) 功能 */
  async SetAvtag(/** 功能id */ id: string /** 禁用或启用 */, avtag: boolean): Promise<void> {
    const config: AxiosRequestConfig = {
      method: 'POST',
...
删除条件：没有在功能价格中引用且在功能店铺中不存在或已到期（已到期会自动删除） */
  async DeleteFunction(/** 功能id */ id: string): Promise<void> {
    const config: AxiosRequestConfig = {
      method: 'POST',
