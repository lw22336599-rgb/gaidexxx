...
import type { GetAuthUrlResult } from '@/TsModel/Alien/Entity/ShopLogin/GetAuthUrlResult'
...
export class JdCallApi {
  ...
  async MessageCallback(
    ...
    format: string,
    v: string,
    jd_param_json: string,
    encrypt_jd_param_json: string
  ): Promise<any> {
    const config: AxiosRequestConfig = {
      method: 'POST',
