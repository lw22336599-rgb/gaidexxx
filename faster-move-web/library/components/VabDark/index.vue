<template>
  <div v-if="'technology' != theme.themeName && 'plain' != theme.themeName && route.path !== '/goods/posterDesign'"
    class="vab-dark-wrapper">
    <el-switch v-model="mode" :active-icon="Moon" active-value="dark" class="vab-dark" :inactive-icon="Sunny"
      inactive-value="light" inline-prompt @click="_toggleDark($event)" />
    <span class="vab-dark-label">{{ mode === 'light' ? '明亮' : '暗黑' }}</span>
  </div>
</template>
